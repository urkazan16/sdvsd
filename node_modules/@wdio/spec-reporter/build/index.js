...
class SpecReporter extends _reporter.default {
  ...
  getTestLink({
    ...
    const isSauceJob = config.hostname.includes('saucelabs') || // only show if multiremote is not used
    config.capabilities && ( // check w3c caps
    config.capabilities['sauce:options'] || // check jsonwp caps
  ...
  getHeaderDisplay(runner) {
    const combo = this.getEnviromentCombo(runner.capabilities, undefined, runner.isMultiremote).trim(); // Spec file name and enviroment information
  ...
  getResultDisplay() {
    ...
    for (const suite of suites) {
      ...
      for (const test of eventsToReport) {
        ...
        output.push(`${testIndent}${this.chalk[this.getColor(state)](this.getSymbol(state))} ${testTitle}`); // print cucumber data table cells
