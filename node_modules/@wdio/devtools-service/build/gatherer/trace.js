...
class TraceGatherer extends _events.default {
  ...
  async startTracing(url) {
    ...
    await page.evaluateOnNewDocument(_registerPerformanceObserverInPage.default);
  ...
  async onFrameNavigated(msgObj) {
    ...
    if ( // we already detected a frameId before
    this.frameId || // the event was thrown for a sub frame (e.g. iframe)
    msgObj.frame.parentId || // we don't support the url of given frame
    !(0, _utils.isSupportedUrl)(msgObj.frame.url)) {
  ...
  async onLoadEventFired() {
    ...
    const loadPromise = Promise.all([this.waitForNetworkIdleEvent.promise, this.waitForCPUIdleEvent.promise]).then(() => () => {
      /**
      ...
      if (minTraceTime > 0) {
        ...
        return new Promise(resolve => setTimeout(() => resolve(this.completeTracing()), minTraceTime));
      }
  ...
  async completeTracing() {
    ...
    try {
      ...
      this.trace = _objectSpread({}, traceEvents, {
  ...
  finishTracing() {
    ...
    this.driver.getCDPSession().then(session => {
      NETWORK_RECORDER_EVENTS.forEach(method => session.removeListener(method, this.networkListeners[method]));
  ...
  waitForNetworkIdle(session, networkQuietThresholdMs = _constants.NETWORK_IDLE_TIMEOUT) {
    ...
    const promise = new Promise(resolve => {
      ...
      const logStatus = () => {
        ...
        log.info(`Found ${inflightRecords.length} inflight network records`); // If there are more than 20 inflight requests, load is still in full swing.
