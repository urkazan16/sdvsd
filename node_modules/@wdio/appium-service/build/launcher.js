...
class AppiumLauncher {
  ...
  async onPrepare(config) {
    ...
    this.logPath = appiumConfig.logPath || config.outputDir; // Set config command
  ...
  _redirectLogStream(logPath) {
    const logFile = (0, _getFilePath.default)(logPath, DEFAULT_LOG_FILENAME); // ensure file & directory exists
  ...
  _cliArgsFromKeyValue(keyValueArgs) {
    ...
    for (let key in keyValueArgs) {
      const value = keyValueArgs[key]; // If the value is false or null the argument is discarded

      if (typeof value === 'boolean' && !value || value === null) {
        continue;
      }

      cliArgs.push(`--${(0, _paramCase.default)(key)}`); // Only non-boolean and non-null values are added as option values
  ...
  _sanitizeCliOptionValue(value) {
    const valueString = String(value); // Encapsulate the value string in single quotes if it contains a white space
